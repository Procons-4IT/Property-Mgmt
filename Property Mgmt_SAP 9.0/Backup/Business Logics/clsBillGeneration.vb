        ...
        Try
            ...
            ElseIf aForm.PaneLevel = 2 Then 'add expenses allocation
                ...
                If oTemp1.RecordCount <= 0 Then
                    ...
                    strSQL = "SELECT isnull(sum( T1.[U_Z_SPACE]),0) FROM [dbo].[@Z_CONTRACT]  T0  inner Join  [dbo].[@Z_PROPUNIT]  T1 "
                    strSQL = strSQL & " on T1.[U_Z_PROITEMCODE]=T0.[U_Z_UNITCODE] where isnull(T0.U_Z_STATUS,'PEN')='AGR' and ('" & strStartDate & "'  between (T0.U_Z_Startdate) and (T0.U_Z_EndDate))"
            ...
            If aForm.PaneLevel = 3 Then
                ...
                Else
                    ...
                    'strSQL = strSQL & " on T1.[U_Z_PROITEMCODE]=T0.[U_Z_UNITCODE]  inner Join OCRD T2 on T2.Cardcode=T0.U_Z_TENCODE where isnull(T0.U_Z_STATUS,'PEN')='AGR' and ( ('" & strStartDate & "'  between (T0.U_Z_Startdate) and (T0.U_Z_EndDate))"
                    'strSQL = strSQL & " or ('" & strEndDate & "'  between (T0.U_Z_Startdate) and (T0.U_Z_EndDate)))"

                    strSQL = "SELECT T0.[DocEntry], T0.[U_Z_UNITCODE], T1.[U_Z_SPACE], T0.[U_Z_TENCODE], T0.[U_Z_ANNUALRENT], T0.[U_Z_PAYTRMS], T0.[U_Z_CHGMONTH],T0.[U_Z_ANNUALRENT]/T0.[U_Z_CHGMONTH],CASE T0.U_Z_TYPE  when 'T' then T0.U_Z_ACCTCODE1 else T0.U_Z_LiaAc  end  ,'N', 0.000,0.000 ,T0.[U_Z_ConNo] 'ContrctID' ,T0.U_Z_ProType 'PropertyType',T0.U_Z_CommAc 'CommissionAccount',T0.U_Z_Comm 'ComPer', T0.U_Z_OwnerCode 'Owner' ,T0.U_Z_SeqNo 'Seq' ,T0.U_Z_CntNo FROM [dbo].[@Z_CONTRACT]  T0  inner Join  [dbo].[@Z_PROPUNIT]  T1 "
                    strSQL = strSQL & " on T1.[U_Z_PROITEMCODE]=T0.[U_Z_UNITCODE]  inner Join OCRD T2 on T2.Cardcode=T0.U_Z_TENCODE where isnull(T0.U_Z_STATUS,'PEN')='AGR' and ( ('" & strStartDate & "'  between (T0.U_Z_Startdate) and (T0.U_Z_EndDate))"
                    strSQL = strSQL & ")"
                    ...
                    strSQL = strSQL & " on T1.[U_Z_PROITEMCODE]=T0.[U_Z_UNITCODE]  inner Join OCRD T2 on T2.Cardcode=T0.U_Z_TENCODE where isnull(T0.U_Z_STATUS,'PEN')='AGR' and "
                    strSQL = strSQL & "  ('" & strEndDate & "'  between (T0.U_Z_Startdate) and (T0.U_Z_EndDate))"
                    ...
                    If blnLineExists = False Then
                        ...
                        Return False
                    End If
                    oHeaderGrid.DataTable.ExecuteQuery("Select * from [@Z_OBILL] where  U_Month=" & intMonth & " and U_Year=" & intYear & " ORDER BY U_ContractID")
                    Dim strsql12 As String
                    strsql12 = "SELECT T0.[Code], T0.[Name], T0.[U_YEAR], T0.[U_MONTH], T0.[U_CONTRACTID], T0.[U_CONTRACTNUMBER], T0.[U_SEQ], T0.[U_UNITCODE], T0.[U_SPACE], T0.[U_CARDCODE], T0.[U_PAYTRMS], T0.[U_ANNUALRENT], T0.[U_CHGMONTH], T0.[U_MONTHRENT], T0.[U_RENTGL], T0.[U_EXPENSES], T0.[U_TOTAL], T0.[U_INVOICED], T0.[U_INVENTRY], T0.[U_INVNUMBER], T0.[U_Z_PROTYPE], T0.[U_REMARKS], T0.[U_COMPER], T0.[U_COMMISSION], T0.[U_COMMGL], T0.[U_OWNERCODE] FROM [dbo].[@Z_OBILL]  T0"
                    strsql12 = strsql12 & "  where  U_Month=" & intMonth & " and U_Year=" & intYear & " ORDER BY U_ContractID"
        ...
        Try
                ...
                Else
                    ...
                    If 1 = 2 Then ' oTemp.Fields.Item("U_Z_ProType").Value = "T" Then
                        ...
                        Else
                            ...
                            otemp2.DoQuery("Select * from OINV where Docentry=" & strDocNum)
                            otemp3.DoQuery("Update [@Z_OBILL] set U_Invoiced='Y' , U_InvEntry=" & strDocNum & ",U_InvNumber=" & otemp2.Fields.Item("DocNum").Value & " where Code='" & oTemp.Fields.Item("Code").Value & "'")
                            WriteErrorlog("Invoice created successfully Contract Number : " & oTemp.Fields.Item("U_ContractNumber").Value & ": Invoice Number : " & otemp2.field.item("DocNum").value, sPath)
                            WriteErrorlog("Invoice created successfully Contract Number : " & oTemp.Fields.Item("U_ContractNumber").Value & ": Invoice Number : " & otemp2.field.item("DocNum").value, sPath)
                        End If
                    End If
                End If
            ...
            If blnErrorflag = True Then
                If oApplication.Company.InTransaction() Then
                    oApplication.Company.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack)
                End If
                WriteErrorlog("Process completed with errors, All the transactions created are rolled back. ", sFailureLog)
        ...
        Select Case aOption
            ...
            Case "Payroll"
                ...
                Try
                    ...
                    oCombo.ValidValues.Add("A", "Owned by AQAR")
                    oCombo.ValidValues.Add("T", "Owned by 3rd Party")
                    oCombo.DisplayType = SAPbouiCOM.BoComboDisplayType.cdt_both
                                ...
                                Select Case pVal.ItemUID
                                    ...
                                    Case "5"
                                        ...
                                        If GenerateInvoice(oForm) = True Then
                                            ...
                                            sPath = System.Windows.Forms.Application.StartupPath & "\FImportLog_Invoice.txt"
                                            x.FileName = sPath
