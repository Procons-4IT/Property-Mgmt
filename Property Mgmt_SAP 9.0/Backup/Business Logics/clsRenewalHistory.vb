Public Class clsRenewalHistory
    Inherits clsBase
    ...
    Public Sub LoadForm(ByVal acode As String, ByVal aChoice As String)
        oForm = oApplication.Utilities.LoadForm(xml_RenewalHistory, frm_RenewalHistory)
        ...
        If aChoice = "Booking" Then
            ...
            Dim oteset As SAPbobsCOM.Recordset
            oteset = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oteset.DoQuery("Select isnull(U_Z_Master,U_Z_CONNO),* from [@Z_CONTRACT] where DocEntry=" & acode)
            Dim strMainContract As String = oteset.Fields.Item(0).Value
            If strMainContract = "" Then
                strMainContract = oteset.Fields.Item("U_Z_Conno").Value
            End If

            strString = "SELECT T0.[DocEntry], case when convert(varchar,T0.U_Z_SeqNo)<>'' then T0.U_Z_CONNO +'_' + Convert(varchar,T0.U_Z_SeqNo) else T0.U_Z_CONNO end 'U_Z_CONNO',T0.[U_Z_UNITCODE], T0.[U_Z_DESC], T0.[U_Z_CONTDATE],case T0.[U_Z_STATUS] When 'PED' then 'Pending for approval' when 'APP' then 'Approved' when 'AGR' then 'Agreed' when 'TER' then 'Terminated' else 'Cancelled' end 'Status', T0.[U_Z_STARTDATE],T0.[U_Z_ENDDATE], T0.[U_Z_TENCODE], T0.[U_Z_TENNAME], T0.[U_Z_ANNUALRENT], T0.[U_Z_BASECONNO],T0.U_Z_BaseSeq 'U_Z_BASESEQNO', T0.[U_Z_BASESTARTDATE], T0.[U_Z_BASEENDDATE] FROM [dbo].[@Z_CONTRACT]  T0"
            strString = strString & " where (U_Z_CONNO ='" & strMainContract & "' or U_Z_Master='" & strMainContract & "') order by U_Z_StartDate"
            oApplication.Utilities.setEdittextvalue(oForm, "6", oteset.Fields.Item("U_Z_CONNO").Value)
            oApplication.Utilities.setEdittextvalue(oForm, "7", oteset.Fields.Item("U_Z_TENCODE").Value)
            oApplication.Utilities.setEdittextvalue(oForm, "8", oteset.Fields.Item("U_Z_TENNAME").Value)
        End If
        ...
        If aChoice <> "Activity" Then
            ...
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_CONNO")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_2")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_UNITCODE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_3")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_DESC")
            '  oColumn.Type = SAPbouiCOM.BoFormItemTypes.it_LINKED_BUTTON
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_4")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_CONTDATE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_5")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_STARTDATE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_6")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_ENDDATE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_7")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_TENCODE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_8")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_TENNAME")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
            oColumn = oMatrix.Columns.Item("V_9")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_ANNUALRENT")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_10")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_BASECONNO")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_13")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_BASESEQNO")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_11")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_BASESTARTDATE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_12")
            oColumn.DataBind.Bind("oMatrixDT", "U_Z_BASEENDDATE")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)

            oColumn = oMatrix.Columns.Item("V_16")
            oColumn.DataBind.Bind("oMatrixDT", "Status")
            oColumn.TitleObject.Sortable = True
            oColumn.TitleObject.Sort(SAPbouiCOM.BoGridSortType.gst_Ascending)
...
#Region "DataBind"
    Private Sub DataBind(ByVal aform As SAPbouiCOM.Form)
        ...
        End If
        If strToBP <> "" Then
            strSql = strSql & " and X.CardCode <='" & strfromBP & "'"
        ...
        strString = strString & " union all Select U_Z_ContID 'U_Z_ContID' ,U_Z_ContNumber 'ContractNumber','RC' 'TransType',DocEntry,DocNum,CardCode,CardName,DocDate,Doctotal from ORCT where ( (isnull(U_Z_ContID,'')<>''  or U_Z_ContNumber<>''))"
        ...
        strSQL3 = " Select 0,sum(CheckSum) 'DocTotal1' from OCHO T3 inner Join [@Z_CONTRACT]  T4 on T4.U_Z_CONNO=T3.TransRef where " & stCondition1
        strSQl2 = " Select 0,sum(LocTotal) 'DocTotal1' from ODPS  T0 inner Join [@Z_CONTRACT] T4 on T4.U_Z_CONNO =T0.U_Z_CONTNUMBER  where  (isnull(U_Z_ContID,'')<>'' or U_Z_ContNumber<>'') and " & stCondition1
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_RenewalHistory Then
                Select Case pVal.BeforeAction
                    ...
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_MATRIX_LINK_PRESSED
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "11" And (pVal.ColUID = "V_0" Or pVal.ColUID = "V_1") Then
                                    oMatrix = oForm.Items.Item("11").Specific
                                    Dim oobj As New clsTenContracts
                                    oobj.LoadForm_Contract_View(oApplication.Utilities.getMatrixValues(oMatrix, "V_1", pVal.Row))
                                End If
