        ...
        Try
            ...
            Select Case strreportChoice
                ...
                Case "O" 'with out contract
                    strsql = "select U_Z_PROPDESC,U_Z_PROITEMCODE,U_Z_CODE,T1.U_Z_DESC,T0.U_Z_UNITTYPE,case U_Z_UNITSTATUS when 'O' then 'Offered' when 'R' then 'Reserved (before signature)' when 'L' then 'Reserved (After Signature)' when 'S' then 'Sold' when 'A' then 'Available' else 'Not Available' end,U_Z_SPACE,U_Z_PRICE,U_Z_TOTALAREA,U_Z_TOTALPRICE,U_Z_COSTCENTER from [@Z_PROPUNIT] T1 "
