Public Class clsReportWizard
    ...
    Public Sub LoadForm(ByVal acode As String, ByVal aChoice As String)
        ...
        If aChoice = "Booking" Then
            acode = "1"
            strContractNumber = "1"
            strString = "Select U_Z_ContID, U_Z_CntNumber 'ContractNumber','DP'  'TransType',DocEntry,DocNum,CardCode,CardName,DocDate,Doctotal from ODPI where 1=2"
            strsql1 = "Select  sum(t1.LineTotal) 'Doctotal',0 from OINV T0 inner Join INV1 T1 on T1.DocEntry=T0.DocEntry  where 1=2 ) and isnull(U_Z_InvType,'T')='T'"
...
#End Region
#Region "DataBind"
    Private Function DataBind(ByVal aform As SAPbouiCOM.Form) As Boolean
        oForm = aform
        ...
        Try
            ...
            Dim oTest As SAPbobsCOM.Recordset
            Dim achoice, acode, strString, strSql, strfromBP, strToBP, strFromBooking, strToBooking, strContrFrom, strContrTo As String
            oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            ...
            strContrFrom = oApplication.Utilities.getEdittextvalue(aform, "edFrom")
            strContrTo = oApplication.Utilities.getEdittextvalue(aform, "edTo")
            ...
            If strfromBP <> "" Then
                ...
                strCheck = "T1.CardCode >='" & strfromBP & "'"
            ...
            If strToBP <> "" Then
                ...
                strCheck = strCheck & " and T1.CardCode <='" & strfromBP & "'"
            Else
                strSql = strSql & " and 2=2"
                stCondition = stCondition & " and 2=2"
                stCondition1 = stCondition1 & " and 2=2"
                strCheck = strCheck & " and 2=2"
            End If

            If strContrFrom <> "" Then
                strSql = strSql & " and  X.U_Z_ContID >=" & strContrFrom '& "'"
                stCondition = stCondition & " and T0.U_Z_ContID >=" & strContrFrom
                stCondition1 = stCondition1 & " and  T4.DocEntry >=" & strContrFrom

                strCheck = strCheck & " and  T0.[U_Z_CONTID] >=" & strContrFrom

            Else
                strSql = strSql & " and 1=1"
                stCondition = stCondition & " and 1=1"
                stCondition1 = stCondition1 & " and 1=1"
                strCheck = strCheck & " and 1=1"
            End If

            If strContrTo <> "" Then
                strSql = strSql & " and  X.U_Z_ContID <=" & strContrTo '& "'"

                stCondition = stCondition & " and T0.U_Z_ContID <=" & strContrTo
                stCondition1 = stCondition1 & " and  T4.DocEntry <=" & strContrTo
            ...
            strString = "Select X.U_Z_ContID,X.ContractNumber,X.TransType,X.DocEntry,X.DocNum,X.CardCode,X.CardName,X.DocDate,X.DocTotal from (" & strString & ") x  where " & strSql & "  order by  X.U_Z_ContID,X.TransType,X.CardCode,X.DocEntry,X.DocDate"
            ...
            strSQl2 = "Select Sum(X.x),SUm(X.DocTotal1) from (" & strSQl2 & ") X"
            strsql4 = " Select 0,sum(cashsum+creditsum+trsfrsum)  'DocTotal1' from ORCT T0  where checksum<=0 and (isnull(U_Z_ContID,'')<>'' or U_Z_ContNumber<>'' ) And " & stCondition
            ...
            oGrid = aform.Items.Item("40").Specific
            ...
            oTes.DoQuery(strSQL3)
            dblOwnerAmount = oTes.Fields.Item(1).Value
            oApplication.Utilities.setEdittextvalue(oForm, "36", dblCreditNote)
            oApplication.Utilities.setEdittextvalue(oForm, "edDep", dblDepositedamount)
            oApplication.Utilities.setEdittextvalue(oForm, "edInv", dblInvoicedAmount)
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "26" Then
                                    ...
                                    ElseIf oForm.PaneLevel = 4 Then
                                        oForm.PaneLevel = oForm.PaneLevel - 2
                                    Else
                                        oForm.PaneLevel = oForm.PaneLevel - 1
                                    End If
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        If pVal.ItemUID = "edFrom" Or pVal.ItemUID = "edTo" Then
                                            val2 = oDataTable.GetValue("DocEntry", 0)
                                            val = oDataTable.GetValue("U_Z_CNTNO", 0)
                                            If pVal.ItemUID = "edFrom" Then
                                                oApplication.Utilities.setEdittextvalue(oForm, "33", val)
                                            Else
