...
#Region "Methods"
    ...
    Private Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_Contracts) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_Contracts, frm_Contracts)
...
#End Region
    ...
    Private Function ValidateUnitExists(ByVal aform As SAPbouiCOM.Form, ByVal dtStartdate As Date, ByVal dtEndDate As Date) As Boolean
        ...
        If aform.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
            ...
            'strSql = "Select * from [@Z_CONTRACT_OWN] where U_Z_UnitCode='" & strUnitCode & "' and DocEntry<>" & intDocEntry & " and '" & dtStartdate.ToString("yyyy-MM-yy") & "' between U_Z_StartDate and U_Z_EndDate"
            strSql = "Select * from [@Z_CONTRACT_OWN] where U_Z_UnitCode='" & strUnitCode & "' and  U_Z_STATUS<>'CAN'  and DocEntry<>" & intDocEntry & " order by DocEntry desc"
            ...
            If oTemp.RecordCount > 0 Then
                ...
                End If
            End If
            'strSql = "Select * from [@Z_CONTRACT_OWN] where U_Z_UnitCode='" & strUnitCode & "' and DocEntry<>" & intDocEntry & " and '" & dtEndDate.ToString("yyyy-MM-yy") & "' between U_Z_StartDate and U_Z_EndDate"
            'oTemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            'oTemp.DoQuery(strSql)
            'If oTemp.RecordCount > 0 Then
            '    oApplication.Utilities.Message("Contract already exits for the selected Unit Code. The contract start date should be greater than previous contract end date", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
...
#Region "AddRow /Delete Row"
    Private Sub AddRow(ByVal aForm As SAPbouiCOM.Form)
        ...
        Select Case aForm.PaneLevel
            Case "1"
                oMatrix = aForm.Items.Item("50").Specific
                oDataSrc_Line = oForm.DataSources.DBDataSources.Item("@Z_CONTRACT_OWN1")
            Case "3"
                oMatrix = aForm.Items.Item("62").Specific
                oDataSrc_Line = oForm.DataSources.DBDataSources.Item("@Z_CONTRACT_OWN2")
        End Select
        Try
            aForm.Freeze(True)
    ...
    Private Function validation(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        If aform.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
            ...
            If oApplication.Utilities.getEdittextvalue(oForm, "9") = "" Then
                oApplication.Utilities.Message("Owner code is missing for ", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If

            If oApplication.Utilities.getEdittextvalue(aform, "58") = "" Then
                oApplication.Utilities.setEdittextvalue(aform, "58", oApplication.Utilities.getEdittextvalue(aform, "9"))
                '  oApplication.Utilities.Message("Owner code is missing for ", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                '  Return False
            Else

            End If
            ' oApplication.Utilities.setEdittextvalue(aform, "9", oApplication.Utilities.getEdittextvalue(aform, "58"))
        ...
        If oCombo.Selected.Value = "T" Then
            ...
            End If
            If oApplication.Utilities.getEdittextvalue(aform, "58") = "" Then
                oApplication.Utilities.Message("Owner code is missing for Tenant Contract Type", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
        End If
...
#End Region
    ...
    Private Function CreateDownPaymentInvoice(ByVal acode As Integer) As Boolean
        ...
        'otemp1.DoQuery("Select isnull(U_Z_DPDocentry,0) from [@Z_CONTRACT_OWN] where DocEntry=" & acode)
        ...
        otemp.DoQuery("select isnull(U_Z_Annualrent,0), * from [@Z_CONTRACT_OWN] where DocEntry=" & acode)
        ...
        If dbldownpayment > 0 Then
            ...
            otemp.DoQuery("select isnull(U_Z_Annualrent,0) , isnull(U_Z_Type,'O') 'Type', * from [@Z_CONTRACT_OWN] where DocEntry=" & acode)
            If otemp.Fields.Item(0).Value > 0 Then
                ...
                contractNumber = otemp.Fields.Item("U_Z_ConNo").Value
                ...
                Else
                    ...
                    Else
                        ...
                        dbltotalDownpayment = 0
                    End If
                    otemp1.DoQuery("Update [@Z_CONTRACT_OWN] set U_Z_DPEntry=" & otemp.Fields.Item(0).Value & ",U_Z_DPNumber='" & otemp.Fields.Item(1).Value & "' where docentry=" & acode)
    ...
    Private Function CreateSecurityDepost(ByVal acode As Integer) As Boolean
        ...
        End If
        Dim strsql, strType, strOwner As String
        otemp.DoQuery("select isnull(U_Z_Deposit,0), * from [@Z_CONTRACT_OWN] where DocEntry=" & acode)
        dbldownpayment = otemp.Fields.Item(0).Value
        If blnRecordExits = True Then
            dbldownpayment = dbldownpayment - dbltotalDownpayment
        End If
        If dbldownpayment > 0 Then
            ' otemp.DoQuery("select isnull(U_Z_Deposit,0) + isnull(U_Z_Salik,0)+isnull(U_Z_DPAmount,0), * from [@Z_ORDR] where DocEntry=" & acode)
            otemp.DoQuery("select isnull(U_Z_Deposit,0), isnull(U_Z_Type,'O') 'Type', * from [@Z_CONTRACT_OWN] where DocEntry=" & acode)
            If otemp.Fields.Item(0).Value > 0 Then
                strcustCode = otemp.Fields.Item("Type").Value
                If strcustCode = "O" Then
                    strOwner = otemp.Fields.Item("U_Z_TenCode").Value
    ...
    Public Sub RightClickEvent(ByRef eventInfo As SAPbouiCOM.ContextMenuInfo, ByRef BubbleEvent As Boolean)
        ...
        If oForm.TypeEx = frm_Contracts Then
            If (eventInfo.BeforeAction = True) Then
                ...
                Try
                    If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then
                        ...
                        oCreationPackage.UniqueID = "InvList"
                        ...
                        oCreationPackage = oApplication.SBO_Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams)
                        oCreationPackage.Type = SAPbouiCOM.BoMenuType.mt_STRING
                        oCreationPackage.UniqueID = "Agreement"
            ...
            Else
                ...
                Try
                    If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then
                        oApplication.SBO_Application.Menus.RemoveEx("InvList")
                        oApplication.SBO_Application.Menus.RemoveEx("Agreement")
                    End If
                Catch ex As Exception
                    MessageBox.Show(ex.Message)
                End Try
        ...
        Try
            Select Case pVal.MenuUID
                ...
                Case "Agreement"
                    ...
                    If oForm.TypeEx = frm_Contracts Then
                        ...
                        If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE And strNo <> "" Then
                            ...
                            End Try
                            If stType = "T" Then
                                oObj.PrintContract_Tenant(strNo, "Contract")
                            Else
                ...
                Case mnu_ADD
                    oForm = oApplication.SBO_Application.Forms.ActiveForm()
                    If pVal.BeforeAction = False Then
                        oApplication.Utilities.setEdittextvalue(oForm, "1000002", oApplication.Utilities.getMaxCode("@Z_CONTRACT_OWN", "DocEntry"))
...
#Region "Get Next Number"
    Private Sub GetNextNumber(ByVal aForm As SAPbouiCOM.Form)
        Dim oTest As SAPbobsCOM.Recordset
        Dim strType, stQuery As String
        If aForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
            oCombo = aForm.Items.Item("54").Specific
            strType = oCombo.Selected.Value
            oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oTest.DoQuery("Select count(*) +1 from [@Z_CONTRACT_OWN] where U_Z_Type='" & strType & "'")
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_Contracts Then
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                ...
                                If (pVal.ItemUID = "4" Or pVal.ItemUID = "6") And pVal.CharPressed = 9 Then
                                    ...
                                    If strFromDate <> "" And strToDate <> "" Then
                                        oApplication.Utilities.setEdittextvalue(oForm, "28", DateDiff(DateInterval.Month, dtFromDate, dttoDate) + 1)
                                    End If
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        ElseIf pVal.ItemUID = "58" Then
                                            ...
                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "48", val1)
                                                oApplication.Utilities.setEdittextvalue(oForm, pVal.ItemUID, val)
                                            Catch ex As Exception
                                                PopulateCustomerAddress(val)
                                            End Try
                                        ...
                                        ElseIf pVal.ItemUID = "49" Or pVal.ItemUID = "56" Or pVal.ItemUID = "72" Then
