Public Class clsTenContracts
    ...
    Private oCombo, oCombo1 As SAPbouiCOM.ComboBox
...
#Region "Methods"
    ...
    Private Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_TenContracts) = False Then
        ...
        oEditText.ChooseFromListUID = "CFL_UNIT"
    ...
    Public Sub LoadForm1(ByVal ContCode As String)
        ...
        oForm.PaneLevel = 1
        oForm.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE
        oApplication.Utilities.setEdittextvalue(oForm, "82", ContCode)
...
#End Region
    ...
    Private Function ValidateUnitExists(ByVal aform As SAPbouiCOM.Form, ByVal dtStartdate As Date, ByVal dtEndDate As Date) As Boolean
        ...
        If aform.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
            Dim intDocEntry As Double
            Dim strUnitCode, strSql As String
            Dim adate As Date
            intDocEntry = oApplication.Utilities.getDocumentQuantity(oApplication.Utilities.getEdittextvalue(aform, "1000002"))
            ...
            strSql = "Select * from [@Z_CONTRACT] where U_Z_UnitCode='" & strUnitCode & "' and  (U_Z_STATUS<>'CAN' and U_Z_STATUS<>'TER')  and DocEntry<>" & intDocEntry & " order by DocEntry desc"
...
#Region "DataBind"
    Private Sub databind(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            oCombo = aForm.Items.Item("74").Specific
            Try
                For intRow As Integer = oCombo.ValidValues.Count - 1 To 0 Step -1
                    oCombo.ValidValues.Remove(intRow, SAPbouiCOM.BoSearchKey.psk_Index)
                Next
            ...
            End Try
            aForm.Items.Item("74").DisplayDesc = True
...
#Region "validations"
    Private Function validation(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        Else
            ...
            '    oApplication.Utilities.Message("Downpayment Account  is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '    Return False
            'End If
        End If

        strAnnualRent = oApplication.Utilities.getEdittextvalue(aform, "19")
        If strAnnualRent = "" Then
            ' oApplication.Utilities.Message("Annual rent is missing....", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            'Return False
        Else
            dblAnnualrent = oApplication.Utilities.getDocumentQuantity(strAnnualRent)
            If dblAnnualrent < 0 Then
                oApplication.Utilities.Message("Security Deposit  should be greater than zero...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
            'If oApplication.Utilities.getEdittextvalue(aform, "49") = "" And dblAnnualrent <> 0 Then
            '    oApplication.Utilities.Message("Security Deposit Account  is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '    Return False
            'End If
        End If
        ...
        If oApplication.Utilities.getEdittextvalue(aform, "56") = "" Then
            oApplication.Utilities.Message("Revenue account code is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End If
        ...
        If oCombo.Selected.Value = "T" Then
            ...
            If oApplication.Utilities.getEdittextvalue(aform, "58") = "" Then
                ...
                Return False
            End If
            oCombo = aform.Items.Item("95").Specific
            Dim iscommreq As Boolean = False
            Try
            ...
            Catch ex As Exception
                iscommreq = False
            End Try
            If iscommreq = True Then
                If oApplication.Utilities.getDocumentQuantity(oApplication.Utilities.getEdittextvalue(aform, "edComm")) <= 0 Then
                    oApplication.Utilities.Message("Commission rate is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If
            End If

            If iscommreq = True And oApplication.Utilities.getEdittextvalue(aform, "76") = "" Then
                oApplication.Utilities.Message("Commission Account missing for Tenant Contract Type", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
        End If
        ...
        Try
            ...
            For intLoop As Integer = 1 To oMatrix.VisualRowCount
                ...
                End If
            Next
            Dim strdocnum, sQuery As String
            oCombo = aform.Items.Item("21").Specific
            If oCombo.Selected.Value = "TER" And aform.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Then
                Dim otest As SAPbobsCOM.Recordset
                otest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                strdocnum = oApplication.Utilities.getEdittextvalue(aform, "1000002")
                If strdocnum <> "" Then
                    otest.DoQuery("select * from [@Z_CONTRACT]  where DocEntry=" & strdocnum)
                    If otest.RecordCount > 0 Then
                        Dim intTempID As String = oApplication.Utilities.GetTemplateID(oForm, "TER")
                        If intTempID <> "0" Then
                            oApplication.Utilities.Message("Termination need the approval.Create Termination transaction to Terminate Contract.", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                            Return False
                        End If
                    End If
                End If
            End If
            oCombo = aform.Items.Item("106").Specific
            oCombo1 = aform.Items.Item("21").Specific
            If oCombo.Selected.Value = "A" Then
                If oCombo1.Selected.Value = "PED" Then
                    oApplication.Utilities.Message("Contract already approved. You can choose either agreed/cancelled status.", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If
            End If
        Catch ex As Exception
...
#End Region
    ...
    Private Function CreateDownPaymentInvoice(ByVal acode As Integer) As Boolean
        ...
        If oApplication.SBO_Application.MessageBox("Do you want to create Downpayment document ?", , "Yes", "No") = 2 Then
            Return True
        End If
        Dim dbltotalDownpayment, dbldownpayment, dblCRAmount As Double
        ...
        otemp1.DoQuery("SELECT sum(DocTotal)-sum(VatSum)  FROM ORIN T0 WHERE T0.[U_Z_DPType]='A' and T0.[U_Z_ContID] =" & acode)

        dblCRAmount = otemp1.Fields.Item(0).Value
        dbltotalDownpayment = dbltotalDownpayment - dblCRAmount

        If dbltotalDownpayment > 0 Then 'otemp1.RecordCount > 0 Then
            dbltotalDownpayment = dbltotalDownpayment 'otemp1.Fields.Item(0).Value
            blnRecordExits = True
        Else
        ...
        If dbldownpayment > 0 Then
            ...
            If otemp.Fields.Item(0).Value > 0 Then
                Dim dtDPDate As Date = otemp.Fields.Item("U_Z_DPDATE").Value
                ...
                If otemp.Fields.Item("U_Z_ProType").Value = "T" Then
                    ...
                    strCreditAccountcode = otemp.Fields.Item("U_Z_DPAcctCode").Value
                Else
                    ' strCreditAccountcode = otemp.Fields.Item("U_Z_LiaAc").Value
                    strCreditAccountcode = otemp.Fields.Item("U_Z_DPAcctCode").Value
                ...
                If dtDPDate.Year <> 1899 Then
                    oDoc.TaxDate = dtDPDate
                    oDoc.DocDate = dtDPDate
                    oDoc.DocDueDate = dtDPDate
                Else
    ...
    Private Function CreateSecurityDepost(ByVal acode As Integer) As Boolean
        ...
        If dbldownpayment > 0 Then
            ...
            If otemp.Fields.Item(0).Value > 0 Then
                Dim dtDPDate As Date = otemp.Fields.Item("U_Z_SDPDATE").Value
                strcustCode = otemp.Fields.Item("Type").Value
                Dim strCreditAccountcode As String
                If otemp.Fields.Item("U_Z_ProType").Value = "T" Then
                    ' strCreditAccountcode = otemp.Fields.Item("U_Z_LiaAc").Value
                    strCreditAccountcode = otemp.Fields.Item("U_Z_AcctCode").Value
                    'oDoc.UserFields.Fields.Item("U_Z_INVTYPE").Value = "O"
                Else
                    strCreditAccountcode = otemp.Fields.Item("U_Z_LiaAc").Value
                    ' oDoc.UserFields.Fields.Item("U_Z_INVTYPE").Value = "T"
    ...
    Public Sub RightClickEvent(ByRef eventInfo As SAPbouiCOM.ContextMenuInfo, ByRef BubbleEvent As Boolean)
        ...
        If oForm.TypeEx = frm_TenContracts Then
            If (eventInfo.BeforeAction = True) Then
                ...
                Try
                    If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then
                        ...
                        oCreationPackage = oApplication.SBO_Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams)
                        oCreationPackage.Type = SAPbouiCOM.BoMenuType.mt_STRING
                        oCreationPackage.UniqueID = "CheckList"
                        oCreationPackage.String = "Check Transaction Details"
                        oCreationPackage.Enabled = True
                        oMenuItem = oApplication.SBO_Application.Menus.Item("1280") 'Data'
                        oMenus = oMenuItem.SubMenus
                        oMenus.AddEx(oCreationPackage)
                        ...
                        oCreationPackage = oApplication.SBO_Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams)
                        oCreationPackage.Type = SAPbouiCOM.BoMenuType.mt_STRING
                        oCreationPackage.UniqueID = "ConApp"
                        oCreationPackage.String = "Contract Approval History"
                        oCreationPackage.Enabled = True
                        oMenuItem = oApplication.SBO_Application.Menus.Item("1280") 'Data'
                        oMenus = oMenuItem.SubMenus
                        oMenus.AddEx(oCreationPackage)
                        ...
                        oCreationPackage = oApplication.SBO_Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams)
                        oCreationPackage.Type = SAPbouiCOM.BoMenuType.mt_STRING
                        oCreationPackage.UniqueID = "TerApp"
                        oCreationPackage.String = "Termination Approval History"
                        oCreationPackage.Enabled = True
                        oMenuItem = oApplication.SBO_Application.Menus.Item("1280") 'Data'
                        oMenus = oMenuItem.SubMenus
                        oMenus.AddEx(oCreationPackage)
                        oDataSrc_Line = oForm.DataSources.DBDataSources.Item("@Z_CONTRACT")
                        '   MsgBox(oDataSrc_Line.GetValue("U_Z_STATUS", 0).Trim)
                        If oDataSrc_Line.GetValue("U_Z_STATUS", 0).Trim <> "PED" And oDataSrc_Line.GetValue("U_Z_STATUS", 0).Trim <> "CAN" Then

                            If oDataSrc_Line.GetValue("U_Z_APPREQUIRED", 0).Trim = "Y" And oDataSrc_Line.GetValue("U_Z_CONAPPSTATUS", 0).Trim <> "A" Then
            ...
            Else
                ...
                Try
                    If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then
                        If oApplication.SBO_Application.Menus.Exists("InvList1") Then
                            oApplication.SBO_Application.Menus.RemoveEx("InvList1")
                        End If
                        If oApplication.SBO_Application.Menus.Exists("CheckList") Then
                            oApplication.SBO_Application.Menus.RemoveEx("CheckList")
                        End If
                        If oApplication.SBO_Application.Menus.Exists("Renewal") Then
                            oApplication.SBO_Application.Menus.RemoveEx("Renewal")
                        End If
                        If oApplication.SBO_Application.Menus.Exists("ConApp") Then
                            oApplication.SBO_Application.Menus.RemoveEx("ConApp")
                        End If
                        If oApplication.SBO_Application.Menus.Exists("TerApp") Then
                            oApplication.SBO_Application.Menus.RemoveEx("TerApp")
                        End If
                        If oApplication.SBO_Application.Menus.Exists("Agreement1") Then
        ...
        Try
            Select Case pVal.MenuUID
                ...
                Case "TerApp"
                    ...
                    If oForm.TypeEx = frm_TenContracts Then
                        ...
                        If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE And strNo <> "" Then
                            ...
                            oObj.LoadViewHistory("TER", strNo)
                        End If
                    End If
                Case "ConApp"
                    oForm = oApplication.SBO_Application.Forms.ActiveForm
                    If oForm.TypeEx = frm_TenContracts Then
                        ...
                        If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE And strNo <> "" Then
                            Dim oObj As New clsAppHisDetails
                            oObj.LoadViewHistory("TEA", strNo)
                        End If
                    End If
                Case "Renewal"
                    oForm = oApplication.SBO_Application.Forms.ActiveForm
                ...
                Case "CheckList"
                    Dim strcode, strCode1 As String
                    oForm = oApplication.SBO_Application.Forms.ActiveForm
                    strcode = oApplication.Utilities.getEdittextvalue(oForm, "82")
                    Dim strNo As String
                    Try
                        strNo = oForm.Items.Item("82").Specific.value
                    Catch ex As Exception
                        strNo = ""
                    End Try

                    Try
                        strCode1 = oForm.Items.Item("1000002").Specific.value
                    Catch ex As Exception
                        strCode1 = ""
                    End Try
                    If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE And strNo <> "" Then
                        oForm = oApplication.Utilities.LoadForm("frm_CheckStatement.xml", "frm_Check")
                        oForm = oApplication.SBO_Application.Forms.ActiveForm()
                        oApplication.Utilities.setEdittextvalue(oForm, "3", strcode)
                        oApplication.Utilities.setEdittextvalue(oForm, "5", strcode)

                        oApplication.Utilities.setEdittextvalue(oForm, "8", strCode1)
                        oApplication.Utilities.setEdittextvalue(oForm, "9", strCode1)
                ...
                Case mnu_ADD
                    ...
                    If pVal.BeforeAction = False Then
                        ...
                        oForm.Items.Item("edDesc").Enabled = False
                        oForm.Items.Item("82").Enabled = False
...
#End Region
    ...
    Public Sub LoadForm_Contract_View(ByVal aCode As String)
        ...
        ' oApplication.Utilities.setEdittextvalue(oForm, "82", aCode)
        oForm.Items.Item("1").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
        oForm.PaneLevel = 1
        oForm.Freeze(False)
    End Sub

    Public Sub LoadForm_Contract_View_Payment(ByVal aCode As String)
        oForm = oApplication.Utilities.LoadForm(xml_TenContracts, frm_TenContracts)
        oForm = oApplication.SBO_Application.Forms.ActiveForm()
        oForm.Freeze(True)
        oForm.DataBrowser.BrowseBy = "1000002"
        ...
        Try
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD Then
                ...
                If oCombo.Selected.Value = "O" Then
                    ...
                    oForm.Items.Item("49").Visible = True
                    oForm.Items.Item("1000004").Visible = True
                Else
                ...
                If oDataSrc_Line.GetValue("U_Z_RENTTYPE", 0).Trim = "A" Then
                ...
                If oCombo.Selected.Value = "TER" Then
                    ...
                    oForm.Items.Item("77").Enabled = False
                ...
                Else
                    If oDataSrc_Line.GetValue("U_Z_APPREQUIRED", 0).Trim = "Y" And oDataSrc_Line.GetValue("U_Z_CONAPPSTATUS", 0).Trim <> "A" Then
                        ...
                        oForm.Items.Item("60").Enabled = False
                        oForm.Items.Item("77").Enabled = False
                    ElseIf oDataSrc_Line.GetValue("U_Z_TERSTATUS", 0).Trim = "Y" Then
                        oForm.Items.Item("1").Enabled = False
                        oForm.Items.Item("120").Enabled = False
                        oForm.Items.Item("60").Enabled = False
                        oForm.Items.Item("77").Enabled = False
                    Else
                        ...
                        If oCombo.Selected.Value = "PED" Then
                            oForm.Items.Item("77").Enabled = False
                        Else
                            oForm.Items.Item("77").Enabled = True
                        End If
            ...
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD) Then
                ...
                stXML = stXML.Replace("<?xml version=""1.0"" encoding=""UTF-16"" ?><ContractsParams><DocEntry>", "")
                stXML = stXML.Replace("</DocEntry></ContractsParams>", "")
                ...
                If stXML <> "" Then
                    ...
                    If otest.RecordCount > 0 Then
                        ...
                        Else
                            oApplication.Utilities.UpdateApprovalRequired("@Z_CONTRACT", "DocEntry", otest.Fields.Item("DocEntry").Value, "N", intTempID)
                            sQuery = "Update ""@Z_CONTRACT"" Set U_Z_AppStatus = 'Y',U_Z_Status='APP',U_Z_ConAppStatus='A' Where DocEntry=" & stXML
                    ...
                    If stXML <> "" Then
                        ...
                        If otest.RecordCount > 0 Then
                            ...
                            If intTempID <> "0" Then
                                ...
                                'oApplication.Utilities.InitialMessage("Contract Request", otest.Fields.Item("DocEntry").Value, oApplication.Utilities.DocApproval(oForm, "TER"), intTempID, otest.Fields.Item("U_Z_TENNAME").Value, "TER")
                            Else
                                oApplication.Utilities.UpdateApprovalRequired("@Z_CONTRACT", "DocEntry", otest.Fields.Item("DocEntry").Value, "N", intTempID)
                                sQuery = "Update [@Z_CONTRACT] Set U_Z_TERStatus = 'Y' Where  U_Z_Status='TER',U_Z_TerAppStatus='A' and DocEntry = " & stXML
                                otest.DoQuery(sQuery)
                            End If
                        End If
                    End If
                ...
                Select Case pVal.BeforeAction
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "1" And (oForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or oForm.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                                    '   oApplication.Utilities.cmdDraftToOrder_Click(25, "17")
                                ...
                                Select Case pVal.ItemUID
                                    Case "85"
                                        If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then ' SAPbouiCOM.BoFormMode.fm_ADD_MODE And oForm.Mode <> SAPbouiCOM.BoFormMode.fm_FIND_MODE Then
                                            ...
                                            Exit Sub
                                        Else
                                            oApplication.Utilities.Message("Installment details view option available only in OK Mode", SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                                            BubbleEvent = False
                                            Exit Sub
                                        End If
                                    Case "96"
                                        ...
                                        stCode = oApplication.Utilities.getEdittextvalue(oForm, "43")
                                        oObjUnit.LoadForm(stCode)
                                        BubbleEvent = False
                                        Exit Sub
                                End Select
                    ...
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If (pVal.ItemUID = "84" Or pVal.ItemUID = "28" Or pVal.ItemUID = "17") And pVal.CharPressed = 9 Then
                                    ...
                                    dblAnnual = oApplication.Utilities.getDocumentQuantity(oApplication.Utilities.getEdittextvalue(oForm, "84"))
                                    If pVal.ItemUID <> "17" Then
                                        dblMonth = dblAnnual * dblNoMonth
                                        oApplication.Utilities.setEdittextvalue(oForm, "17", dblMonth)
                                    Else
                                        dblNoMonth = oApplication.Utilities.getDocumentQuantity(oApplication.Utilities.getEdittextvalue(oForm, "28"))
                                    ...
                                    Case "120"
                                        If oForm.Mode <> SAPbouiCOM.BoFormMode.fm_ADD_MODE And oForm.Mode <> SAPbouiCOM.BoFormMode.fm_FIND_MODE Then
                                            ...
                                            If oCombo.Selected.Value = "TER" Or oCombo.Selected.Value = "CAN" Then
                                                ...
                                                Exit Sub
                                            End If
                                            If oApplication.Utilities.getEdittextvalue(oForm, "92") = "" Then
                                                oApplication.Utilities.Message("Downpayment Posting Date is missing..", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                                Exit Sub
                                            End If
                                            If oApplication.Utilities.getEdittextvalue(oForm, "98") = "" Then
                                                oApplication.Utilities.Message("Downpayment Invoice Account is missing..", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                                Exit Sub
                                            End If
                                    ...
                                    Case "60"
                                        If oForm.Mode <> SAPbouiCOM.BoFormMode.fm_ADD_MODE And oForm.Mode <> SAPbouiCOM.BoFormMode.fm_FIND_MODE Then
                                            ...
                                            If oCombo.Selected.Value = "TER" Or oCombo.Selected.Value = "CAN" Then
                                                ...
                                                Exit Sub
                                            End If
                                            If oApplication.Utilities.getEdittextvalue(oForm, "49") = "" Then
                                                oApplication.Utilities.Message("Security Deposit Downpayment Account is missing..", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                                Exit Sub
                                            End If
                                            CreateSecurityDepost(CInt(oApplication.Utilities.getEdittextvalue(oForm, "1000002")))
                                        End If
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        ElseIf pVal.ItemUID = "58" Then
                                            val = oDataTable.GetValue("CardCode", 0)
                                            oApplication.Utilities.setEdittextvalue(oForm, pVal.ItemUID, val)
                                        ElseIf pVal.ItemUID = "49" Or pVal.ItemUID = "76" Or pVal.ItemUID = "56" Or pVal.ItemUID = "72" Or pVal.ItemUID = "98" Or pVal.ItemUID = "101" Then
                                            val = oDataTable.GetValue("FormatCode", 0)
                                            oApplication.Utilities.setEdittextvalue(oForm, pVal.ItemUID, val)
                                        ElseIf pVal.ItemUID = "43" Then
                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "43", oDataTable.GetValue("U_Z_PROITEMCODE", 0))
                                            Catch ex As Exception

                                            End Try

                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "edComm", oDataTable.GetValue("U_Z_COMM", 0))
                                            Catch ex As Exception

                                            End Try

                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "edUsedFor", oDataTable.GetValue("U_Z_USEDFOR1", 0))
                                            Catch ex As Exception

                                            End Try
                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "edExtPro", oDataTable.GetValue("U_Z_EXTPRONO", 0))
                                            Catch ex As Exception

                                            End Try
                                            Try

                                                oApplication.Utilities.setEdittextvalue(oForm, "10", oDataTable.GetValue("U_Z_OWNERCODE", 0))
                                            Catch ex As Exception
                                                '   oApplication.Utilities.setEdittextvalue(oForm, "9", strSelectedItem4)
                                                oCombo = oForm.Items.Item("54").Specific
                                                oApplication.Utilities.setEdittextvalue(oForm, "58", oDataTable.GetValue("U_Z_OWNERCODE", 0))
                                                oForm.Items.Item("4").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                                            End Try
                                            oApplication.Utilities.setEdittextvalue(oForm, "edDesc", oDataTable.GetValue("U_Z_PROPDESC", 0))
                                        End If
                                        oForm.Freeze(False)
                                    End If
                                Catch ex As Exception
